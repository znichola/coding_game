...
function planDisrupt(after: Int8Array, myC: Cand[], foeC: Cand[], t0: number) {
    { // HUNTER: keep hitting the region that holds most of the opponent's tracks (mimics the real foe)
        let bestR = -1, bestN = 0;
        for (const [r, cells] of regionCells) {
            if (regionHasTown.has(r) || isInked[cells[0]]) continue;
            let n = 0; for (const c of cells) if (after[c] === 1 - myId) n++;
            if (r === disruptTarget && n > 0) n += 3;
            if (n > bestN) { bestN = n; bestR = r; }
        }
        if (bestR >= 0) { disruptTarget = bestR; lastHit = bestR; cmd.disrupt(bestR); return; }
    }
